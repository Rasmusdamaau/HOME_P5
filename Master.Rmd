...
```{r}
lower <- 0.75 * EL_HOME$Kontantpris
upper <- 1.25 * EL_HOME$Kontantpris
sum(exp(pred) < upper & exp(pred) > lower)

ordered_data <- EL_HOME[order(EL_HOME$Kontantpris),]

yc <- predict(mod5, ordered_data, interval = "confidence")
ypi <- predict(mod5, ordered_data, interval = "prediction")
...
<!-- plot_data <- data.frame(x = seq_along(EL_HOME$Kontantpris), p = exp(pred), k = -->
<!--                           EL_HOME$Kontantpris) -->

<!-- ggplot(plot_data, aes(x, k)) + -->
<!--   geom_point(color = "red") + -->
<!--   geom_point(aes(x, p), color = "blue") + -->
<!--   theme_bw() + -->
<!--   xlab("Index") + -->
<!--   ylab("Pris") -->


<!-- resid_plot <- data.frame(res = EL_HOME$Kontantpris - exp(pred), index = -->
<!--                            seq_along(residuals(mod3))) -->

<!-- ggplot(resid_plot, aes(index, res)) + -->
<!--   geom_point() -->

<!-- summary((residuals(mod3))) -->
<!-- summary(EL_HOME$Kontantpris - exp(pred)) -->
