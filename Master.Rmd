...
# removing Kvartal parameters, and also 
...
EL_HOME$AntalToiletter <- factor(EL_HOME$AntalToiletter)
summary(EL_HOME$AntalPlan)
EL_HOME <- EL_HOME[-which(EL_HOME$AntalPlan == 7),]
EL_HOME$AntalPlan <- factor(EL_HOME$AntalPlan)
...
mod2 <- lm(log(Kontantpris) ~ Boligtilstand + Boligareal + Altan +
             OmbygningSket + Opfoerelsesaar + Salgsaar + AntalPlan, data = EL_HOME)
summary(mod2)
anova(mod1,mod2)
...
mod3 <- lm(log(Kontantpris) ~ Boligareal + Altan + Opfoerelsesaar + Boligtilstand +
             Salgsaar + AntalPlan, data = EL_HOME)
summary(mod3)
anova(mod2,mod3)
# the p-value says that Opfoerelsesaar can safely be removed
# mod3 seems to be the best model
```


# Check mod3

```{r "residual and leverage check of mod3"}
res <- rstudent(mod3)
pred <- predict(mod3)
...
train.control <- trainControl(method = "repeatedcv", 
                              number = 5, repeats = 10)
# Train the model
model <- train(log(Kontantpris) ~ Boligareal + Altan + Opfoerelsesaar +
                 Boligtilstand + Salgsaar + AntalPlan, 
...
# Train the model
model_loocv <- train(log(Kontantpris) ~ Boligareal + Altan + Opfoerelsesaar +
                       Boligtilstand + Salgsaar + AntalPlan,
                     data = EL_HOME, method = "lm",
                     trControl = train.control)
# Summarize the results
