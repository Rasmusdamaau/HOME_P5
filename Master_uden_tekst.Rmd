...
```{r "residuals and leverage3.1", warning=FALSE}
res_large <- which(abs(res) > 4.5)
EL_HOME[res_large,]
```
\newpage

```{r "residuals and leverage3.2", warning=FALSE}
...
```{r "residuals and leverage4.1", warning=FALSE}
hat_large <- which(hat > 0.2)
EL_HOME[hat_large,]
```
\newpage

```{r "residuals and leverage4.2", warning=FALSE}
