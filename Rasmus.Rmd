...
mod1 <- lm(log(Kontantpris) ~ Boligtilstand +Boligareal + AntalToiletter + Altan + OmbygningSket + Opfoerelsesaar + Liggetid + Salgsaar + Alder + Saeson, data = EL_HOME)
summary(mod1)
...
train.control <- trainControl(method = "repeatedcv", 
                              number = 1329, repeats = 1)
# Train the model
model <- train(log(Kontantpris) ~ Boligareal  + Altan + Opfoerelsesaar + Salgsaar, data = EL_HOME, method = "lm",
               trControl = train.control)
# Summarize the results
print(model)
```
